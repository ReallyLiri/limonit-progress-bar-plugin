...
public class ProgressBarUi extends BasicProgressBarUI {
    private final @NotNull BufferedImage backgroundImage;
    ...
    public ProgressBarUi() {
        ...
        try {
            bufferedImage = ImageIO.read(Objects.requireNonNull(this.getClass().getResource("/bricks.png")));
        } catch (IOException e) {
            e.printStackTrace();
            bufferedImage = UIUtil.createImage(null, 0, 0, 0);
        }
    ...
    @Override
    protected void paintIndeterminate(@NotNull Graphics g2d, @NotNull JComponent c) {
    ...
    @Override
    protected void paintDeterminate(@NotNull Graphics g, @NotNull JComponent c) {
        ...
        TexturePaint tp = new TexturePaint(backgroundImage, new Rectangle2D.Double(0, 1, h - 2f * off - off, h - 2f * off - off));
        g2.setPaint(tp);
    ...
    private void paintString(@NotNull Graphics g, int x, int y, int w, int h, int fillStart, int amountFull) {
        if (!(g instanceof Graphics2D)) {
            return;
        }
